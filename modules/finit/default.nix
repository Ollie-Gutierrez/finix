...
in
{
  ...
  options.finit = {
    ...
    package = lib.mkOption {
      ...
      apply =
        package:
        # if config.programs.plymouth.enable or false && lib.versionAtLeast cfg.package.version "4.17" then
        #   package.override {
        ...
        package.overrideAttrs (old: {
          configureFlags =
            ...
            ++ lib.optionals config.programs.plymouth.enable or false [
              "--enable-plymouth-plugin=yes"
            ];

          env = lib.optionalAttrs config.programs.plymouth.enable or false {
